...
std::unique_ptr<AMatrix<std::complex<double>>>
  FastFourierTransformMmd::Forward1d(
  ...
  mmd::MmdVector<std::complex<double>> freq_cplx_vector(
  ...
  return absl::make_unique<AMatrix<std::complex<double>>>(
...
std::unique_ptr<AMatrix<std::complex<double>>>
  FastFourierTransformMmd::Inverse1d(
    const std::unique_ptr<FftManager> &fft_manager,
  ...
  mmd::MmdVector<std::complex<double>> out_cmplx_vector(
      fft_manager->GetSamplesPerChannel());
  for (size_t i=0; i < fft_manager->GetSamplesPerChannel(); i++) {
...
std::unique_ptr<AMatrix<double>> FastFourierTransformMmd::Inverse1dConjSym(
    const std::unique_ptr<FftManager> &fft_manager,
    const AMatrix<std::complex<double>> &in_matrix) {
  ...
  return absl::make_unique<AMatrix<double>>(out_double_vector.size(),
