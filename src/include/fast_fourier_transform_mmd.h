...
class FastFourierTransformMmd {
 public:
   ...
   */
  static std::unique_ptr<AMatrix<std::complex<double>>> Forward1d(
   ...
   * @return the resulting 1D matrix of complex doubles.
   */
  static std::unique_ptr<AMatrix<std::complex<double>>> Inverse1d(
      const std::unique_ptr<FftManager> &fft_manager,
   ...
   */
  static std::unique_ptr<AMatrix<double>> Inverse1dConjSym(
      const std::unique_ptr<FftManager> &fft_manager,
      const AMatrix<std::complex<double>> &in_matrix);
};
}  // namespace Visqol
