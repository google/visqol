...
// correctly.
const AMatrix<double> k65Samples{std::valarray<double>{0.000150529, 5.89739e-05,
...
const AMatrix<std::complex<double>> k65SamplesComplex{
  std::valarray<std::complex<double>>{{0.000150529, 0.0}, {5.89739e-05, 0.0},
  {-9.36187e-05, 0.0}, {-9.36187e-05, 0.0}, {0.000394677, 0.0},
  {0.000303122, 0.0}, {-9.36187e-05, 0.0}, {0.000303122, 0.0},
  {8.94924e-05, 0.0}, {0.000150529, 0.0}, {-2.06314e-06, 0.0},
  {-0.000154656, 0.0}, {2.84554e-05, 0.0}, {0.000272603, 0.0},
  {-0.000185174, 0.0}, {5.89739e-05, 0.0}, {0.000364159, 0.0},
  {0.000425196, 0.0}, {0.000120011, 0.0}, {5.89739e-05, 0.0},
  {0.000120011, 0.0}, {0.000211566, 0.0}, {0.000150529, 0.0},
  {2.84554e-05, 0.0}, {0.000120011, 0.0}, {0.000150529, 0.0},
  {8.94924e-05, 0.0}, {-9.36187e-05, 0.0}, {0.000242085, 0.0},
  {0.000486233, 0.0}, {8.94924e-05, 0.0}, {0.000120011, 0.0},
  {8.94924e-05, 0.0}, {8.94924e-05, 0.0}, {0.000150529, 0.0},
  {-0.000154656, 0.0}, {-2.06314e-06, 0.0}, {2.84554e-05, 0.0},
  {0.000150529, 0.0}, {-3.25817e-05, 0.0}, {5.89739e-05, 0.0},
  {5.89739e-05, 0.0}, {-0.000124137, 0.0}, {5.89739e-05, 0.0},
  {2.84554e-05, 0.0}, {0.000181048, 0.0}, {-0.000124137, 0.0},
  {-0.000368285, 0.0}, {-9.36187e-05, 0.0}, {-0.000246211, 0.0},
  {-3.25817e-05, 0.0}, {2.84554e-05, 0.0}, {-6.31002e-05, 0.0},
  {-9.36187e-05, 0.0}, {-2.06314e-06, 0.0}, {-2.06314e-06, 0.0},
  {-0.000185174, 0.0}, {-0.000124137, 0.0}, {2.84554e-05, 0.0},
  {8.94924e-05, 0.0}, {-0.00027673, 0.0}, {-6.31002e-05, 0.0},
  {-0.000215693, 0.0}, {0.000425196, 0.0}, {8.94924e-05, 0.0}}};
...
const AMatrix<std::complex<double>> k65SamplesForwardFFT{
  std::valarray<std::complex<double>>{{0.00322292904000000, 0.00000000000000},
...
// the input is zero.
const AMatrix<double> k65SamplesZero{std::valarray<double>{0, 0, 0, 0, 0, 0, 0,
...
TEST(FastFourierTransformMmdTest, Forward1d_MatrixContentsRandom) {
  auto fft_manager = absl::make_unique<FftManager>(k65Samples.NumElements());
  auto fft_out_matrix = FastFourierTransformMmd::Forward1d(fft_manager,
      k65Samples);

  std::string fail_msg;
  ASSERT_TRUE(CompareComplexMatrix(*fft_out_matrix,
                                   k65SamplesForwardFFT,
...
TEST(FastFourierTransformMmdTest, Forward1d_Resize_MatrixContentsRandom) {
  auto fft_manager = absl::make_unique<FftManager>(k65Samples.NumElements());
  auto fft_out_matrix = FastFourierTransformMmd::Forward1d(fft_manager,
    k65Samples, k65Samples.NumElements() + 1);
...
TEST(FastFourierTransformMmdTest, FftReconstruct) {
  auto fft_manager = absl::make_unique<FftManager>(k65Samples.NumElements());
  auto fft_forward = FastFourierTransformMmd::Forward1d(fft_manager,
    k65Samples);
  auto fft_inverse = FastFourierTransformMmd::Inverse1d(fft_manager,
    *fft_forward);

  std::string fail_msg;
  ASSERT_TRUE(CompareComplexMatrix(k65SamplesComplex,
                                   *fft_inverse,
                                   kTolerance, &fail_msg)) << fail_msg;
}
...
TEST(FastFourierTransformMmdTest, FftReconstructConjsym) {
  auto fft_manager = absl::make_unique<FftManager>(k65Samples.NumElements());
  auto fft_forward = FastFourierTransformMmd::Forward1d(fft_manager,
      k65Samples);
  auto fft_inverse = FastFourierTransformMmd::Inverse1dConjSym(
      fft_manager, *fft_forward);

  std::string fail_msg;
  ASSERT_TRUE(CompareDoubleMatrix(k65Samples,
                                  *fft_inverse,
                                  kTolerance, &fail_msg)) << fail_msg;
}
...
TEST(FastFourierTransformMmdTest, FftReconstructConjsymZero) {
  auto fft_manager = absl::make_unique<FftManager>(
      k65SamplesZero.NumElements());
  auto fft_forward = FastFourierTransformMmd::Forward1d(fft_manager,
      k65SamplesZero);
  auto fft_inverse = FastFourierTransformMmd::Inverse1dConjSym(
      fft_manager, *fft_forward);
  ...
  std::string fail_msg;
  ASSERT_TRUE(CompareDoubleMatrix(k65SamplesZero,
                                  *fft_inverse,
                                  kTolerance, &fail_msg)) << fail_msg;
}
