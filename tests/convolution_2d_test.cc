...
TEST(Convolution2D, 3x3_2d_conv_test) {
  const std::vector<double> w = {0.0113033910173052, 0.0838251475442633,
  ...
  std::vector<double> m{40.0392, 43.3409, 39.5270, 41.1731, 41.3591, 42.6852,
  ...
  std::vector<double> r{40.6634, 42.8407, 40.6395, 41.0129, 41.5407, 42.4677,
  ...
  auto conv_2d_res = Convolution2D<double>::Valid2DConvWithBoundary(window,
                                                                    matrix);
  std::string fail_msg;
  ASSERT_TRUE(CompareDoubleMatrix(expected_result, conv_2d_res, kTolerance,
      &fail_msg));
}
